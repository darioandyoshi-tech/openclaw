...
function buildPricingContext(config: OpenClawConfig): PricingContext {
  ...
  // Hosted aliases require installed manifest policy. Missing optional metadata
  // must not turn an excluded provider into a policy-free pricing source.
  ...
  const fingerprint = JSON.stringify({
    ...
    normalization: snapshot?.plugins.flatMap((plugin) =>
